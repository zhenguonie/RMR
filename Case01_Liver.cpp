...
void Igl_Show(Mesh& mesh)
{
    ...
    const std::string file_saved = "Liver.off";
    if (!igl::readPLY("PointClouds\\Liver.ply", V_p, F_p))
    {
...
double Activate_Distance(double dis, double down_dis = 5)
{
    if (dis < down_dis)
    {
        return  0;
...
Eigen::MatrixX3d Phase2(Mesh& mesh, const Point_set& PCD, double r = 5, double sq_control = 2.0)
{
    //Parameter initialization and variable space allocation
    unsigned int N = mesh.number_of_vertices();
    unsigned int iik = 0;
...
int main()
{
    ...
    Vector site = Vector(-65, -100, -80);  //Center coordinates
    int R = 10.0;  //Sphere radius
    Mesh mesh = Init_Evolution(site,R);
    
    // Import input point cloud
    Point_set PCD;
    std::ifstream f(CGAL::data_file_path("PointClouds\\Liver.ply"), std::ios_base::binary);
    ...
    double eps_all = 0.3;
    // 3- Regulatory factor
    double wf = 0.42, ws = 0.85, wp = 0.8;
    // 4- The length of mesh edge
    double l_control = 6.5;
    // Iteration
    while (STOP_ITER--)
    {
        // Info.
        ...
        if (STOP_ITER % 2 == 0 || STOP_ITER <= ITER2) {
            ...
            else
            {
                ...
                Isotropic_Remeshing(mesh, l_control, 4);
                Mesh_Smoothing(mesh,3,0.1);
            }
        }

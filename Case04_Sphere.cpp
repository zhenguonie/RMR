...
void Igl_Show(Mesh& mesh)
{
    ...
    const std::string file_saved = "Sphere.off";
    if (!igl::readPLY("PointClouds\\Sphere.ply", V_p, F_p))
    {
...
void Mesh_Smoothing(Mesh& mesh, const unsigned int nb_iterations = 1, const double time = 0.5)
{
...
int main()
{
    ...
    Point_set PCD;
    std::ifstream f(CGAL::data_file_path("PointClouds\\Sphere.ply"), std::ios_base::binary);

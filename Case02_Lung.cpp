...
void Igl_Show(Mesh& mesh)
{
    ...
    const std::string file_saved = "Lung.off";
    if (!igl::readPLY("PointClouds\\Lung.ply", V_p, F_p))
    {
...
Eigen::MatrixX3d Phase2(Mesh& mesh, const Point_set& PCD, double r = 5, double sq_control = 5.0)
{
    //Parameter initialization and variable space allocation
    unsigned int N = mesh.number_of_vertices();
    unsigned int iik = 0;
...
int main()
{
    ...
    Vector site = Vector(-80, -50, -60);  //Center coordinates
    int R = 20.0;  //Sphere radius
    Mesh mesh = Init_Evolution(site, R);

    // Import input point cloud
    Point_set PCD;
    std::ifstream f(CGAL::data_file_path("PointClouds\\Lung.ply"), std::ios_base::binary);

...
void Igl_Show(Mesh& mesh)
{
    ...
    const std::string file_saved = "Square_Torus.off";
    if (!igl::readPLY("PointClouds\\Hole.ply", V_p, F_p))
    {
...
void Mesh_Smoothing(Mesh& mesh, const unsigned int nb_iterations = 1, const double time = 0.02)
{
...
Eigen::MatrixX3d Phase2(Mesh& mesh, const Point_set& PCD, double r = 1.5, double sq_control = 1.0)
{
    //Parameter initialization and variable space allocation
    unsigned int N = mesh.number_of_vertices();
    unsigned int iik = 0;
...
int main()
{
    ...
    Vector site = Vector(1.2, 10, 6);  //Center coordinates
    int R = 1.0;  //Sphere radius
    Mesh mesh = Init_Evolution(site, R);

    // Import input point cloud
    Point_set PCD;
    std::ifstream f(CGAL::data_file_path("PointClouds\\Hole.ply"), std::ios_base::binary);
    ...
    double eps_all = 0.3;
    // 3- Regulatory factor
    double wf = 0.5, ws = 0.5, wp = 0.8;
    // 4- The length of mesh edge
    double l_control = 1.1;
    // Iteration
    while (STOP_ITER--)
    {
        // Info.
